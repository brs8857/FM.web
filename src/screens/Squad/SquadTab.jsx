...
export default function SquadTab({ state, dispatch, clubSeason, revealed, prefs, onDismissNote }) {
  const [target, setTarget] = useState(null);
  const open = (kind, id) => setTarget({ kind, id });
  return (
    <div className={layout.tab}>
      <div className={layout.board}>
        <Chalkboard assignments={state.assignments} bench={state.bench} mode="view" compact onSelect={open} onOpenSheet={open} dispatch={dispatch} />
      </div>
          ...
          <ul className={layout.rows}>
            {state.assignments.map((a) => (
              <TeamSheetRow key={a.slotId} code={a.type} name={a.player?.name ?? "Empty"} meta={a.player ? playerMeta(a.player) : undefined}
                ...
                mark={a.player && a.player.slot !== a.type ? { label: "off", title: `Not a ${POSITION_LABEL[a.type].toLowerCase()}` } : undefined}
